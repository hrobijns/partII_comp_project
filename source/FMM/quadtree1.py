...
class QuadNode:
    def __init__(self, x_min, y_min, x_max, y_max, level=0, parent=None):
        ...
        # for FMM:
        self.neighbors        = []  # near‐field cells at same level
        self.interaction_list = []  # far‐field cells at same level
    ...
    def insert(self, body, max_points):
        """Insert `body` into this node (recursively subdividing to enforce ≤ max_points/leaf)."""
...
class QuadTree:
    def __init__(self, bodies, boundary, max_points=1):
        """
        bodies     : iterable of objects with `.position` (2‐tuple) 
                     and any other data you like (e.g. charge).
        ...
        for b in bodies:
            ...
            if not inserted:
                raise ValueError(f"Body at {b.position} outside boundary")

        # then compute neighbor & interaction lists
    ...
    def _compute_interactions(self):
        """
        For each non‐root node, build .interaction_list of far‐field cells 
